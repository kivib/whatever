...
mu_x1_init = max(X(:,2))/5+4*min(X(:,2))/5;
mu_x2_init = 2*max(X(:,2))/5+3*min(X(:,2))/5;
mu_x3_init = 3*max(X(:,2))/5+2*min(X(:,2))/5;
mu_x4_init = 4*max(X(:,2))/5+1*min(X(:,2))/5;
w = zeros(size(X,1),length(a)); 
mu = [mu_x1_init,mu_y_init;mu_x2_init,mu_y_init;mu_x3_init,mu_y_init;mu_x4_init,mu_y_init];
...
four = find(estimate == 4);
% Plot Examples
subplot(2,3,6);
plot(X(one, 1), X(one, 2), 'x',X(two, 1), X(two, 2), 'o', X(three, 1), X(three, 2), '<', X(four, 1), X(four, 2), '*');
